...
def key_gen(n):
    """
        Generates n private and public key pairs.

        PARAMS
        ------
            n: (int) number of key pairs to be generated.

        RETURNS
        -------
            (ecdsa.keys.Private_keys, ecdsa.keys.Public_keys)
    """
    private_key = [randrange(SECP256k1.order) for i in range(n)]
    public_key = list(map(lambda xi: SECP256k1.generator * xi, private_key))
...
def main():
    ...
    mask = 42336103784098162242257642752456491953648983705977041535233853022539567598254
    # print(mask)
    masked_y = list(map(lambda yi: yi * mask, y))
    # print(masked_y[0])
    masked_pub = SECP256k1.generator * mask
    print(masked_pub)
    ...
    i = 2
    signature = ring_signature(x[i], i, message, masked_y, masked_pub)

    # assert(verify_ring_signature(message, y, *signature))
    # print(verify_ring_signature(message, y, *signature))
    # assert(verify_ring_signature(message, masked_y, *signature, masked_pub))
    print(verify_ring_signature(message, masked_y, *signature, masked_pub))
